...
static void sendmsg(kz_msgbox *mboxp, kz_thread *thp, int size, char *p)
{
  ...
  if (mp == NULL) {
    puts("Error: Failed to allocate memory for message buffer.\n");
    kz_sysdown();
  }
  mp->next       = NULL;
  mp->sender     = thp;
...
static kz_thread_id_t thread_recv(kz_msgbox_id_t id, int *sizep, char **pp)
{
  ...
  if (mboxp->receiver) { /* 他のスレッドがすでに受信待ちしている */
    puts("Error: Another thread is already waiting to receive a message.\n");
    kz_sysdown();
  }
...
static void schedule(void)
{
  ...
  if (i == PRIORITY_NUM) { /* 見つからなかった */
    puts("Error: No ready threads found.\n");
    kz_sysdown();
  }
