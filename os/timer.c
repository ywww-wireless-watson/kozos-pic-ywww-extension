...
// 32bit タイマ
volatile struct pic_timer *ms_tmr;
volatile struct pic_timer *ls_tmr;
unsigned int offset; // 周期タイマセットの瞬間の計測用タイマの値
...
int timer_start(int index)
{
  ...
  // 計測用タイマの値を得る
  if (index == TMR_SECOND_DEVICE)
  {
    // offset = timer_get_tmrx();
    offset = ls_tmr->TMRx;
  }
  timer_expire(index);
  return 0;
...
{
  // ls_tmr->TxCONCLR = PIC_TIMER_TxCON_ON;
  // unsigned int tmrx = (ms_tmr->TMRx  << 16 & 0xFFFF0000) | (ls_tmr->TMRx & 0x0000FFFF);
  // ls_tmr->TxCONSET = PIC_TIMER_TxCON_ON;
  return  ls_tmr->TMRx;;
}
...
{
  ms_tmr = regs[TMR_32MS_DEVICE].tmr;
  ls_tmr = regs[TMR_32LS_DEVICE].tmr;
  ls_tmr->TxCON = PIC_TIMER_TxCON_ON | PIC_TIMER_TxCON_T32_USE_32bit_TIMER | PIC_TIMER_TxCON_TCKPS_CLOCK_PRESCALE_1;
}
